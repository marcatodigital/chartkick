    ...
    if (!ChartjsAdapter && "Chart" in window) {
      ChartjsAdapter = new function () {
        ...
        var addLineAnnotation = function(options, title, colour, orientation, coordinate) {
          ...
          var annotation = {
            id: "line-id-"+(options.annotation.annotations.length+1), // unique ID
            type: 'line',
            mode: orientation, // 'horizontal', 'vertical'
            scaleID: scaleID,
            value: coordinate, // the spot on the X/Y axes to place the annotation
            borderColor: colour, // HTML colour to draw the line in
            borderWidth: 1,
            label: {
          ...
          options.annotation.annotations.push(annotation);
        };
